...
Standard_Real AutLib::SizeMap2d_FullIntSurface::DimSize() const
{
	Debug_Null_Pointer(MetricMap());
	return MetricMap()->MaxDim();
}

AutLib::Pnt2d 
AutLib::SizeMap2d_FullIntSurface::CorrEffRegion
(
...
) const
{
	...
	static const Standard_Real Tol = 0.01;
	auto P = theP;
...
Standard_Real 
AutLib::SizeMap2d_FullIntSurface::CalcDistance
(
	const Pnt2d & theP0,
	const Pnt2d & theP1
) const
{
	Mesh_LineIntegrand_FullIntSurface line(theP0, theP1, *this);

	Mesh_LineIntegrand_Function_FullIntSurface func(line);

	Debug_Null_Pointer(Info());
	auto& info = *Info()->AdaptIntgInfo();

	Numeric_AdaptIntegration_FullIntSurface Integration(func, 0, 1, info);
	Integration.Perform();
...
Standard_Real 
AutLib::SizeMap2d_FullIntSurface::CalcUnitDistance
(
	const Pnt2d & theP0,
	const Pnt2d & theP1
) const
{
	...
	Mesh_LineIntegrand_UnitLengthFunction_FullIntSurface func(line);

	Debug_Null_Pointer(Info());
	auto& info = *Info()->AdaptIntgInfo();

	Numeric_AdaptIntegration_UnitLengthFullIntSurface Integration(func, 0, 1, info);
	Integration.Perform();
...
#include <SizeMap2d_Surface_OptmPointStd.hxx>
#include <SizeMap2d_Surface_OptmPoint_IterCorrect.hxx>
#include <Entity2d_Eigen.hxx>

AutLib::Pnt2d 
AutLib::SizeMap2d_FullIntSurface::CalcOptimumCoord
(
	const Standard_Real theSize,
	const front & theEdge
) const
{
	SizeMap2d_Surface_OptmPointStd IntP(theSize, BoundingBox(), theEdge, *this);
	IntP.Perform();
...
AutLib::SizeMap2d_FullIntSurface::box 
AutLib::SizeMap2d_FullIntSurface::CalcSearchRegion
(
	const Standard_Real theRadius,
...
{
	Debug_Null_Pointer(size_map2d_full_surface_adapt_eigen);
	auto& eigen = *size_map2d_full_surface_adapt_eigen;
...
AutLib::SizeMap2d_FullIntSurface::box  
AutLib::SizeMap2d_FullIntSurface::CalcEffectiveRegion
(
	const Standard_Real theRadius,
...
AutLib::SizeMap2d_FullIntSurface::metric 
AutLib::SizeMap2d_FullIntSurface::CalcEffectiveMetric
(
	const Pnt2d & theP0,
	const Pnt2d & theP1
) const
...
AutLib::Pnt3d 
AutLib::SizeMap2d_FullIntSurface::CalcCoord3D
(
...
) const
{
	...
	Debug_Null_Pointer(metricMap.Patch());
	const auto& patch = *metricMap.Patch();

	Pnt3d Pt = patch.Value(theCoord.X(), theCoord.Y());
	return std::move(Pt);
}

void AutLib::SizeMap2d_FullIntSurface::Make()
{
	if (NOT SurfaceSizeMap())
	{
...
AutLib::size_map2d_full_surface_adapt_eigen = std::make_shared<AutLib::Entity2d_Eigen>();