...
namespace AutLib
{
	...
	void Mesh2d_PoleEdgeTemplate<BndEdgeType>::SingularityContraction
	(
	...
	)
	{
		...
		const auto n0 = std::dynamic_pointer_cast<Mesh2d_PoleNodeTemplate<typename BndEdgeType::traitsType::nodeType>>(this->Node0());
		const auto n1 = std::dynamic_pointer_cast<Mesh2d_PoleNodeTemplate<typename BndEdgeType::traitsType::nodeType>>(this->Node1());
		...
		std::shared_ptr<Mesh2d_PoleNodeTemplate<typename BndEdgeType::traitsType::nodeType>> fixed, other;
		...
		edge->SetCharLength(theSizeMap.CalcDistance(edge->Node0()->Coord(), edge->Node1()->Coord()));
		edge->SetCentre(theSizeMap.CalcCentreOf(edge->Node0()->Coord(), edge->Node1()->Coord()));

		Debug_If_Condition(fixed->NbBoundaryEdges() NOT_EQUAL 2);
	}

	template<class BndEdgeType>
	std::vector<std::shared_ptr<typename Mesh2d_PoleEdgeTemplate<BndEdgeType>::edgeType>> 
		Mesh2d_PoleEdgeTemplate<BndEdgeType>::GetTopology
		(
		...
		)
	{
		...
		std::vector<std::shared_ptr<Mesh2d_PoleNodeTemplate<bndNodeType>>> nodes;
		nodes.reserve(pts.size());

		Standard_Integer K = 0;
		for (const auto& x : pts)
		{
			nodes.push_back(std::make_shared<Mesh2d_PoleNodeTemplate<bndNodeType>>(++K, x));
		}
		...
		for (const auto& x : connect)
		{
			...
			auto edge = std::make_shared<Mesh2d_PoleEdgeTemplate>(++K, n0, n1);
			Debug_Null_Pointer(edge);
