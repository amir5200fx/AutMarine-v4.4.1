...
void AutLib::Geo_ApprxSurfMetric::Perform()
{
	if (NOT theSurface_)
	{
		FatalErrorIn(FunctionSIG) << endl
			<< "the algorithm has not been loaded!" << endl
			<< abort(FatalError);
	}
	...
	Info()->ChangeMaxDeterminant() = maxDet;
	if (NOT TrackMaxDeterminant()) Info()->ChangeMaxDeterminant() = 1.0;

	Change_IsDone() = Standard_True;
}

std::shared_ptr<AutLib::Geo_ApprxSurfMetricInfo>
AutLib::surface_metric_approximator_info = std::make_shared<AutLib::Geo_ApprxSurfMetricInfo>();